...
### 🐛 修复与优化
- 更新详情保留完整 Markdown 标题、列表、段落和代码，不再将说明压成一段或截断；长内容独立滚动，发布页和下载按钮固定可见。
- 更新详情、Markdown 语法帮助、发送确认和工具搜索统一弹窗布局、深浅主题、Esc 关闭、Tab 焦点循环与关闭后焦点恢复。
- 发送确认展示完整已有文本，长内容自动换行；自动更新等待当前弹窗关闭后显示，搜索不再叠加其他弹窗。
- 保留短系统消息、研讨费用确认、原有下载选择和更新检查频率。

### 🧪 验证
- 312 项测试、生产构建与深浅主题/最小窗口弹窗烟测通过。

### 📦 安装说明
- macOS 包未使用 Apple 开发者证书签名，首次运行请右键 → 打开。
- 旧版客户端提示本次更新时仍使用旧弹窗；安装本版本后，新弹窗用于后续更新检查。
...
### ✨ 新增
- 全局工具搜索：Cmd/Ctrl+K 搜索工具与操作，方向键选择、Enter 打开、Esc 关闭。
- 会话草稿保护：数据子工具独立保留输入结果、文本处理保留各操作结果、生成器恢复结果；切换已命名文件保留未保存草稿。
- 跨工具发送遇到已有输入时，可选择替换、追加或取消；输入或配置改变后的旧结果标记「待更新」。

### 🎨 优化
- 按编辑器、数据、文本、编码、时间、生成、研讨室七类任务组织导航，设置与主题入口独立；第二行直达具体工具，保留全部现有功能与数字快捷键。
- 编辑器默认首个 tab，目录、编辑、预览保持左右三栏，目录树支持上下/左右滚动查看大量文件和完整长文件名。
- 统一页面标题和输入/结果层级，适配深浅主题与 800×600 窗口；时间转换方向独立切换，Cron 与多时区按任务进入。
- 研讨室准备页集中仓库、想法与启动操作，连接设置按需展开，原 Agent 编排、只读约束、IPC 和记录行为保持不变。

### 🧪 测试与文档
- 新增草稿、异步读取/保存、搜索与发送覆盖保护回归；306 项测试和生产构建通过。
- 同步项目文档与 OpenSpec；`doc_local/` 用于本地原型、方案与 review 产物，默认不进入 Git。
...
### ✨ 新增
- 工具间“发送到”管道：输出面板增加“发送到...”菜单，可直接将结果送入其他工具输入区
- 编辑器打开 .json/.xml/.sql/.yaml/.csv 文件时显示“在数据工具中打开”按钮，一键跳转数据工具
- context-bar 溢出渐变指示：子工具超出可视区域时两端显示渐变遮罩提示可滚动

### 🎨 优化
- “文本调试”分组重命名为“文本工具”，语义更准确
- “编码”工具从“数据处理”组移入“文本工具”组，与正则/对比等调试工具同组
- 文本处理“统计”子工具降级为输入面板常驻 meta 行（实时显示字符·词·行数），子工具精简为 3 个

### 🧪 测试与文档
- 同步 README / CLAUDE.md / AGENTS.md 导航与组件描述；全量 295 个单测与 Electron 生产构建通过。
...
### 🎨 时间工具子工具合并
- 「当前时间」并入「时间转换」：实时当前时间与时间戳概览常驻时间转换页顶部（秒/毫秒切换、一键复制），子工具由 4 个减为 3 个（时间转换、Cron、多时区）。
- 高频路径缩短：获取当前时间戳从切 tab 或 3 步操作变为打开时间转换页即见即复制。
- 宽屏双列 dashboard 布局不变（实时卡仍居左列顶部）；窄屏实时卡随时间转换子任务呈现。

### 🧪 测试与文档
- 同步 README / CLAUDE.md / AGENTS.md 时间工具描述；OpenSpec change `merge-time-current-into-convert` 归档，delta 合并至 `tool-surface-refresh` specs。
...
### 🎨 工作台导航重构
- 一级入口重组为顶部「工作区 / 数据处理 / 文本调试 / 生成工具 / AI / 系统」导航；多工具分组改用左侧上下文工具栏，单工具分组直接使用完整操作区。
- 保留 `Cmd/Ctrl+1–9/0`、`Ctrl+Tab` / `Ctrl+Shift+Tab`，并记忆各分组最后使用的工具，鼠标和键盘入口始终同步。
- 编辑器文件树缩窄为更合适的工作区比例，加入稳定缩进、低干扰结构线和展开文件夹状态，目录层级更易扫描。
- 生成器内部工具改为紧凑横向栏，UUID 等简单配置收紧结果空态，减少无效大面板与空白占用。

### 🧪 测试
- 扩展导航元数据测试，覆盖工具到分组的映射及默认分组入口。
- 完整通过 27 个测试文件、294 个用例与 Electron 生产构建；完成深色、浅色、单/多工具分组、`Ctrl+Tab` 与紧凑窗口真实桌面烟测。
...
### ✨ 新功能
- 设置页“检查更新”现在会根据当前操作系统与 CPU 架构匹配 GitHub Release 安装包；确认后直接在系统浏览器中下载，未找到匹配包时可查看发布页全部附件。
- 新增默认关闭的“启动时每天检查一次更新”选项；启用后仅在发现新版时提醒，不会重复打扰。

### 🛠️ 改进
- 更新比较版本由主进程读取，GitHub 请求增加 10 秒超时与 API 限流、响应异常等清晰提示。
- 附件下载仅接受 GitHub 官方 HTTPS 地址，并覆盖 macOS DMG、Windows EXE 与 Linux AppImage 的平台架构匹配。

### 🧪 测试
- 新增 Release 附件匹配、URL 安全校验、预发布拒绝、限流、超时与每日启动检查间隔测试。
...
### ✨ 新功能
- Workbench 左侧导航在约 900px 以下自动收起，并支持当前会话手动覆盖；macOS 数字直达统一使用 `Cmd+1–9/0`，全平台循环切换统一使用 `Ctrl+Tab` / `Ctrl+Shift+Tab`，不再拦截系统 `Cmd+Tab`。
- 新增可复用的键盘友好“更多操作”菜单、分段控件方向键切换、全局短反馈状态与统一焦点样式，补齐 selected、pressed、disabled、status 和内部滚动区的可访问语义。
- 时间工具重组为当前时间、时间转换、Cron、多时区四个子工具；Cron 首次进入即展示合法默认表达式的解释和未来 5 次执行时间。
- Regex 结果区支持指针拖动、上下方向键调整与恢复默认；完整输入签名确保 debounce 或 Worker 乱序期间不再显示旧输入的高亮与结果。

### 🎨 界面与交互
- 编辑器将列表、编辑、预览改为可并存面板开关，并把新建、Markdown 导出与帮助动作收敛到上下文菜单。
- 数据工具、文本处理、编码、生成器和文本对比统一主操作、结果面板、空态、复制/清空位置及紧凑窗口布局，减少重复状态条与工具栏拥挤。
- Agent 研讨室仅基于现有前端状态重排为“准备 → 运行 → 结果”三阶段，突出启动、进度、时间线与历史记录；IPC、只读编排和持久化语义保持不变。
- 设置页按常用设置、最近文件、快捷键、关于分区；深浅主题的小号文字、placeholder、行号和 scrollbar 对比度同步优化。

### 📚 规格与文档
- 新增可访问 Workbench 交互主规格，并同步 Workbench shell、工具页、Regex 与 Cron 规格；OpenSpec 严格校验 24 份主规格全部通过。
- 同步 README、CLAUDE.md 与 AGENTS.md 的页面结构、响应式导航和快捷键说明。

### 🧪 测试
- 新增菜单、分段控件、全局状态、导航快捷键、Regex 输入快照与分隔条、Cron 初始预览、Agent 前端阶段派生测试。
- 完整通过 27 个测试文件、288 个用例与三 bundle 生产构建，并完成深色/浅色、14px/18px、1200×800/800×600 的十页面真实 Electron 烟测。

### 📦 安装提示
- macOS 安装包尚未签名；首次启动如被 Gatekeeper 拦截，请在 Finder 中对 OneApp 右键 → 打开。

## [1.22.4] - 2026-08-11

### 🐛 修复
- 文本对比结果新增“编辑内容”入口，返回输入态时保留两侧文本，可修改后继续发起下一轮对比。
- 文本 A、文本 B 支持单侧清空并自动聚焦目标编辑器，不再需要清空另一侧内容。
- 文件加载和文本交换改为显式刷新差异结果，避免输入阶段重复执行完整差异计算。

### 🧪 测试
- 新增 DiffTab 交互测试，覆盖返回编辑、内容保留、单侧清空与聚焦、文件替换、交换方向和视图切换。
- 完整通过 22 个测试文件、273 个用例与生产构建，并完成深色、浅色及窄窗口 UI 回归。

## [1.22.3] - 2026-07-12

### 🐛 修复
- 左侧导航的数字快捷键按从上到下的展示顺序统一为 `1–9、0`，避免导航顺序与快捷键提示不一致。
- 收起左侧导航后仅保留工具图标；快捷键数字改由悬浮提示提供。

## [1.22.2] - 2026-07-12

### 🐛 修复
- 修复 macOS Launchpad 中 OneApp 图标四角显示为黑色方形的问题；图标资源现使用透明 alpha 通道，并同步更新 macOS `.icns`、Windows `.ico` 与通用 PNG。
- 将图标背景统一为更亮的深海军蓝，移除原有上下分层的黑色托盘感，保留 `C` 标识与蓝色环形结构。

## [1.22.1] - 2026-07-12

### 🎨 图标
- 全面更新 OneApp 应用图标：以字母 `C` 为核心，结合环形结构与命令提示符意象，提升桌面与 Dock 小尺寸下的辨识度。
- 同步生成 macOS `.icns`、Windows 多尺寸 `.ico`、通用 PNG 与图标预览资源。

## [1.22.0] - 2026-07-05

### ✨ 新功能
- **Agent 研讨室 · 网络/代理注入**：新增「网络 / 代理」配置区（启用开关 / 代理 URL / 可选 `ALL_PROXY`，实时校验并持久化），把代理环境变量稳定注入 Codex / ClaudeCode 研讨子进程，解决 Dock 启动时主进程不继承终端代理变量导致无法连接服务的问题；关闭代理时清理继承而来的代理变量，保持「关闭即无代理」语义明确。
- **Agent 研讨室 · 按需连接测试**：为每个已安装 agent 新增「测试连接」，用与正式研讨相同的代理环境派生逻辑执行最小实调验证连通；不替代登录态检测、不在自动检测阶段消耗服务用量、不写入研讨时间线，运行中受运行互斥保护。
- 只读安全边界保持不变：Codex read-only sandbox、ClaudeCode print + plan 模式与只读工具白名单在代理注入后依旧生效。
...
### ✨ 新增
- 时间工具新增多时区对照表，预置本地、纽约、伦敦、东京、上海和悉尼。
- 支持从预置列表添加或移除城市，并实时显示当前日期、时间和今天 / 明天 / 昨天差异。

### 📚 文档
- 更新 README、ROADMAP、OpenSpec 和剩余功能队列，标记全部剩余功能完成。
- 新增 Timezone Comparison Table OpenSpec，并同步 workbench shell 时间工具摘要规格。

### 🧪 测试
- 扩展 time helper 单测，覆盖时区格式化、日期差异、可添加列表和无效时区错误。
- 更新 navigation metadata 测试，锁定时间工具对多时区能力的说明。
