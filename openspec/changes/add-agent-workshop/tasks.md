...
- [ ] 1.1 Define Agent 研讨室 constants for agent ids, display names, phase ids, statuses, default per-invocation timeout (600s), output limits (512 KB stored/display, 20000 chars injected into downstream prompts), and app-store keys
...
- [ ] 1.4 Implement fixed prompt builders for Round 1 independent proposal, Round 2 cross-review/self-review, and Final moderator summary with repeated read-only constraints, plan-only text output, and no prompting to exit read-only/plan mode
...
- [ ] 2.3 Implement Codex and ClaudeCode detection using a login shell to resolve executable paths and version commands, plus login-state probes (`claude auth status` JSON `loggedIn`, `codex login status` exit code) to mark each agent installed / logged-out / ready
...
- [ ] 2.6 Implement a generic agent invocation runner with stdin prompt writing, stdout/stderr collection, timeout handling (600s default), cancellation handling, output truncation (512 KB stored / 20000-char downstream), and process-tree termination
...
- [ ] 3.2 Expose a single `onAgentDiscussionEvent(callback)` subscription that returns an unsubscribe function for `agent-discussion:event`, with no generic channel access
- [ ] 3.3 Keep the exposed API narrow and avoid exposing generic command execution to the renderer

## 4. Renderer State And UI

- [ ] 4.1 Add the Agent 研讨室 tab (after 编码, before 设置) to `App.vue`, `Header.vue`, extend tab shortcuts to Ctrl+1–8, and update status labels
...
- [ ] 4.5 Implement Codex/ClaudeCode status cards with installed / logged-out / ready states, disabled state with login hints for unavailable or logged-out agents, manual re-detection, and default selection of ready agents
...
- [ ] 5.1 Add tests for ready-agent filtering (installed + logged-in), selected-agent defaults, moderator default/fallback, start validation, and call-count estimation
...
- [ ] 7.2 Update AGENTS.md and CLAUDE.md architecture notes for the new main-process services, renderer components, and event IPC pattern
