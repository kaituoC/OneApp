...
- 所有分组（含工作区、生成器、AI 与系统等单工具分组）在左侧恒定展示「工具 + 子工具」两级导航，页面内不再设第三层导航；多工具分组的子工具（如 JSON/YAML/CSV、Base64/URL/JWT 等）直接在左侧切换
...
- **目录树浏览**：侧边栏内像文件管理器一样浏览子目录、切换目录根、直接点开文件；缩进、结构线和展开文件夹图标清晰表达层级
