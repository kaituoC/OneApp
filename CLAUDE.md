...
OneApp 是一个基于 Electron + Vue 3 的桌面应用程序，提供开发者工具：统一编辑器（按文件后缀自动切换 Markdown / HTML / 纯文本模式）、数据工具（JSON / YAML / CSV / SQL / XML / JSONPath）、文本差异对比、文本处理、生成器（UUID / 随机密码 / Lorem）、时间转换、正则测试、编码工具合集（Base64 / URL / JWT / Hash / 进制 / Unicode）、Agent 研讨室（多个本地 AI agent 在只读模式下研讨本地仓库并汇总实现方案）和设置页 GitHub Release 更新检查。编辑器侧边栏提供懒加载目录树，可像文件管理器一样浏览目录并打开文件。
