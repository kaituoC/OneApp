...
多功能开发工具桌面应用，集成统一编辑器（Markdown / HTML 自动识别）、数据工具（JSON / YAML / CSV / SQL / XML / JSONPath）、文本对比、文本处理、生成器（含二维码）、时间与 Cron、正则测试、编码工具合集、Agent 研讨室和应用级更新检查。
...
### 工作台导航
- 顶部以「编辑器 / 数据 / 文本 / 编码 / 时间 / 生成 / 研讨室」七类任务组织入口，设置、搜索与主题按钮独立
- 第二行直达具体工具（如 JSON/YAML/CSV、Base64/URL/JWT），编辑器、研讨室和设置直接进入内容；`Cmd/Ctrl+K` 可搜索工具与操作
- 数字直达、`Ctrl+Tab` / `Ctrl+Shift+Tab` 与分组最近工具记忆保持同步，返回分组可继续上次工作
...
- **目录树浏览**：侧边栏内像文件管理器一样浏览子目录、切换目录根、直接点开文件；缩进与展开文件夹图标清晰表达层级
- **系统文件对话框**：可打开工作目录外的任意 `.md` / `.html` / `.htm` 文件
- 编辑器行号显示，可分别隐藏文件列表、编辑器或预览区
- 文件列表、编辑区和预览区是可并存的独立面板；新建与 Markdown 导出使用支持完整键盘操作的上下文菜单
...
- 差异统计：新增、删除、修改行数
- 支持水平/垂直滚动同步
- 对比后可返回编辑并保留两侧内容，支持单侧清空后继续下一轮对比
...
- 二维码：输入文本或 URL 生成 PNG 预览，支持尺寸、纠错级别、下载和复制 PNG
- 纯前端离线生成，结果支持一键复制

### 时间工具
- 按「时间转换 / Cron / 多时区」三个子工具组织，切换时保留当前页面生命周期内的输入和结果
- 时间转换页顶部常驻实时当前时间与时间戳概览（秒/毫秒切换、一键复制）
- 时间戳转日期（支持秒/毫秒，多种输出格式）
- 日期转时间戳（同时输出秒和毫秒）
- Cron 表达式解释器：支持标准 5 位 Cron，首次进入即展示合法默认表达式的可读解释和未来 5 次本地时间执行点
- 多时区对照表：预置常用城市，可添加/移除并显示今天/明天/昨天日期差异
- 一键复制结果
...
- 前端按「准备 → 运行 → 结果」三阶段组织：准备阶段聚焦配置与启动，运行阶段聚焦进度和时间线，结果阶段集中历史记录、导出与新研讨入口
...
- **网络 / 代理**：可为研讨调用配置全局代理（启用开关 / 代理 URL / 可选 `ALL_PROXY`），解决 Dock 启动时主进程不继承终端代理变量的问题；关闭时清理继承的代理变量。另提供按需「测试连接」，用与正式研讨相同的代理注入执行最小实调验证连通（不消耗于自动检测、不写入研讨记录）
- 开始前提示预计调用次数与首次成本（会真实调用本地 CLI、消耗对应服务用量）
- 平台：目前仅 **macOS / Linux** 可用，Windows 暂显示「暂不支持」

### 设置与更新
- 设置页按常用设置、最近文件、快捷键和关于分区，展示当前版本、构建日期、主题与字号
- 新版本详情完整显示更新说明的标题、列表与代码，内容独立滚动，底部保留稍后、发布页与下载入口；语法帮助、发送确认和搜索采用相同弹窗交互
- 「检查更新」由主进程读取当前运行版本并请求 GitHub Releases 最新正式版本；有新版时会匹配当前系统与架构的安装包，可直接在浏览器中下载，未匹配时回退到发布页
- 可选“启动时每天检查一次更新”（默认关闭）；请求超时、GitHub 限流或异常响应会给出明确提示
...
编辑器默认目录、编辑、预览三栏，目录树支持双向滚动查看大量文件和完整长文件名。切换已打开文件保留未保存草稿；数据、文本和生成子工具保留本次会话状态。输入或配置变化时，旧结果显示「待更新」，跨工具发送遇到已有输入可选择替换、追加或取消。
...
| Ctrl/Cmd + W | 关闭当前文件 |
| Ctrl/Cmd + R / F5 | 刷新页面 |
| Ctrl + Tab | 切换下一个工具（macOS 同样使用 Ctrl，保留系统 Cmd+Tab） |
| Ctrl + Shift + Tab | 切换上一个工具 |
| macOS：Cmd + 1~9 / 0；Windows/Linux：Ctrl + 1~9 / 0 | 切换到指定工具 |
| F12 | 打开/关闭调试工具 |
