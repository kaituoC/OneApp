...
### 工作台导航
- 顶部以「工作区 / 数据处理 / 文本调试 / 生成工具 / AI / 系统」六个一级分组组织全局入口
- 顶部两行导航：第一行为「工作区 / 数据处理 / 文本调试 / 生成工具 / AI / 系统」六个一级分组，第二行 context-bar 横向导航条内固定段切换分组内工具、动态段切换当前工具的子工具（如 JSON/YAML/CSV、Base64/URL/JWT 等）；单工具分组无侧栏直达内容，页面内不再设第三层导航
